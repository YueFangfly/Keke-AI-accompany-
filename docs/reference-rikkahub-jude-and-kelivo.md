...
27. **世界书四件事** —— 一次改动、一块代码，合起来还是 S 规模：
...
29. **`{{idleDuration}}` 宏 + 稳定的 `{{pick}}`** —— 陪伴向的宏。
    注意 `pick` 必须对同一条消息稳定，否则「重新生成」会变成两个世界
30. **Continue（接着上一条往下写）** —— 治 `max_tokens` 截断。
    **不做 impersonate**：让 AI 替你说话等于取消了关系里的另一方
31. **按引用关系找孤儿文件** —— 碎片流水线上线之后会变必要。
    现在删图是靠 `FragmentStore.delete` 里手工写的一行，靠自觉不靠机制

**语音（2026-09-11 新增）**
32. **TTS 换供应商 / 多供应商** —— 现在只接了 ElevenLabs，而它的**中文有明显
    「翻译腔」**，成语和长句的重音经常不对。对一个说中文的陪伴角色，
    这是能不能用的问题，不是好不好听的问题。
    接入成本很低：全项目只有 4 个 `ElevenLabsService.speech` 调用点 + 1 个
    `voices` 调用点，照搬搜索适配层（§2.13）那套「一个协议 + 每家一个适配器」即可。
    候选见 §13。
33. **默认音色是 `Rachel`（一个英文音色）** —— `ElevenLabsService.defaultVoiceID`
    写死的是英文女声。就算用户填了 key，开箱第一次朗读就是错的。
    要么换成中文音色，要么**不给默认值、强制先选一个**
